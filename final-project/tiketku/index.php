...
<?php
	...
	$host= "host=localhost";
	$port= "port=5432";
	$dbname="dbname=tiketku";
	$dbuser="user=postgres";
	$dbpwd="password=gudanggaram";
...
<?php
  ...
  $result = pg_query($dbconn, "select total_harga AS jumlah_booking,tanggal_booking from booking; ") or exit("Error with quering database");
  ...
  // creating an associative array to store the chart attributes
	$arrData = array(
        "chart" => array(
            ...
            "toolTipColor"=> "#4D394B",
            "plotToolText"=> "<div style='text-align: center; line-height: 1.5;'>\$label<br>\$value Boking<div>",
...
// print_r(pg_fetch_array($result));
	...
	while($row = pg_fetch_array($result)) {
		array_push($arrData["data"], array(
			"label" => $row["tanggal_booking"],
			"value" => $row["jumlah_booking"]
			)
		);
	}
  ...
  $jsonEncodedData = json_encode($arrData);
	...
	$postgresChart = new FusionCharts("column2d", "bokingChart" , '100%', '450', "postgres-chart", "json", $jsonEncodedData);
